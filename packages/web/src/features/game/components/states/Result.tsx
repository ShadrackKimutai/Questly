  ...
  if (feedback.type === "estimate") {
    ...
    return (
      <div className={`mt-2 w-full max-w-md space-y-3 rounded-2xl p-4 backdrop-blur-sm ${tierColor}`}>
        ...
        <div className="flex flex-col gap-1.5 text-sm font-semibold">
          <div className="flex items-center justify-between">
            <span className="text-white/60">{t("game:estimate.yourGuess")}</span>
            <span className="font-bold text-white">
              {feedback.playerAnswer !== null ? feedback.playerAnswer : t("game:noAnswer")}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-white/60">{t("game:estimate.correctAnswer")}</span>
            <span className="font-bold text-emerald-300">{feedback.correctAnswer}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-white/60">{t("game:estimate.offset")}</span>
            <span className="font-bold text-white">
              {feedback.offset !== null
                ? `${feedback.offset > 0 ? "+" : ""}${feedback.offset.toFixed(2).replace(/\.?0+$/, "")}`
                : "—"}
            </span>
          </div>
        </div>
...
const Result = ({
  ...
  return (
    <section className="anim-show relative mx-auto flex min-h-0 w-full max-w-7xl flex-1 touch-pan-y flex-col items-center justify-center gap-2 overflow-y-auto px-4 py-4">
      ...
      {answerFeedback &&
        (answerFeedback.type === "wordcloud" ||
          answerFeedback.type === "dotmocracy" ||
          answerFeedback.type === "estimate" ||
